...
export default function Axios() {
    ...
    const onQueryStringClickHandler = () => {
        ...
        axios.get("http://127.0.0.1:8080/request-data/request-param",
            {
    ...
    const onRequestBodyClickHandler = () => {
        ...
        // 객체 타입이어야 함
        // ! post, patch, put 메서드의 두 번째 매개변수는 반드시 requestBody이다.
        axios.post("http://127.0.0.1:8080/request-data/request-body",
            {name, age}
        ).then((response) => {
            const responseBody = response.data;
            setResponseBody(responseBody);
    ...
    // - 각각의 http method 메서드는 매개변수로 URL, requestBody, option을 받을 수 있다.
    // - get, delete : URL, option [선택]
    // - post, put, patch : URL, requestBody, option
    const onBasicGet = () => {
      axios.get("http://127.0.0.1:8080/basic")
      // 각각의 http method 메소드에 then으로 response 객체를 받는 콜백 함수를 전달
      // then은 request에 대해서 성공인 경우 수행
      // then은 response의 http 상태 코드 100 ~ 300으로 판단
       .then((response) => {
