...
// root 경로에 있는 index.tsx의 root.render()안의 <App/> 컴포넌트를
// <BrowserRouter> 컴포넌트로 감싸야함
// <BrowserRouter>: URL을 사용하여 브라우저 주소 표시줄에 현재 위치를
// 저장하고 탐색할 수 있도록 하는 컴포넌트

// <Routes>: <Route> 컴포넌트로 URL에 따른 컴포넌트를 렌더링 할 수 있도록 하는 컴포넌트
// <Route> : URL 패턴에 따라서 렌더링하고자 하는 
// - path 속성 : URL 패턴 지정 
// - element 속성 : 렌더링할 컴포넌트를 지정
// - index 속성 : 현재 경로의 기본 라우터로 지정

// react에서 '/'는 host 주소의 바로 뒤를 의미하기 때문에 Route 내부의 Route path 경로에는
// '/'를 지워야한다.
