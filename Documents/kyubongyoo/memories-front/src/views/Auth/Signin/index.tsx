...
export default function SignIn(props: Props) {
  ...
  const onLoginButtonClick = () => {
    if(!userId) setUserIdMessage('아이디를 입력하지 않았습니다.');
    if(!userPassword) setUserPasswordMessage('비밀번호를 입력하지 않았습니다.');
  ...
  return (
    <div id='auth-login-container'>
        <div className='header'>Memories</div>
          ...
          <InputBox label='아이디' value={userId} placeholder='아이디를 입력해주세요.' type='text' message={userIdMessage} isErrorMessage onChange={onUserIdChangeHandler}/>
          <InputBox label='비밀번호' value={userPassword} placeholder='비밀번호를 입력해주세요.' type='password' message={userPasswordMessage} isErrorMessage onChange={onUserPasswordChangeHandler}/>
        </div>
